...
if (convertFilesBtn && themeSelectorFile) {
    convertFilesBtn.addEventListener('click', async () => {
        ...
        const chooseSaveLocation = document.getElementById('choose-save-location').checked;

        // Update theme
        themeStylesheet.href = getStylesheet(selectedThemeFile);

        if (conversionMode === 'separate') {
            await convertSeparatePDFs(chooseSaveLocation);
        } else {
            await convertMergedPDF(chooseSaveLocation);
        }
    });
}

// Convert each file to separate PDF
async function convertSeparatePDFs(chooseSaveLocation = false) {
    const totalFiles = uploadedFiles.length;
    let saveDirectory = null;

    // If user wants to choose save location, prompt for directory
    if (chooseSaveLocation) {
        try {
            ...
            if ('showDirectoryPicker' in window) {
                ...
                alert('Your browser does not support choosing save location. Files will be saved to default download folder.');
            }
        } catch (error) {
    ...
    for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        await convertSingleFileToPDF(file, saveDirectory);
...
async function convertSingleFileToPDF(file, saveDirectory = null) {
    // Update code display
    ...
    try {
        ...
        if (saveDirectory) {
            // Save to user-selected directory with folder structure
            await savePDFToDirectory(pdf, pdfPath, saveDirectory);
        } else {
            // Default download - replace slashes with underscores for filename
...
// Helper function to save PDF to user-selected directory with folder structure
async function savePDFToDirectory(pdf, filePath, directoryHandle) {
    try {
...
async function convertMergedPDF(chooseSaveLocation = false) {
    let saveDirectory = null;
    ...
    if (chooseSaveLocation) {
        try {
            if ('showDirectoryPicker' in window) {
                ...
                alert('Your browser does not support choosing save location. File will be saved to default download folder.');
            }
        } catch (error) {
            if (error.name !== 'AbortError') {
                console.error('Error selecting directory:', error);
    ...
    try {
        ...
        // Download merged PDF
        if (saveDirectory) {
            await savePDFToDirectory(pdf, 'Merged-Code.pdf', saveDirectory);
